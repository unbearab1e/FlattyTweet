// Type: MetroTwit.ViewModel.AccountManagementViewModel
...
namespace MetroTwit.ViewModel
{
  public class AccountManagementViewModel : MultiAccountViewModelBase
  {
    ...
    public string URL
    {
        ...
        set
        {
            if (string.Equals(this.url, value, StringComparison.Ordinal))
                return;
            this.url = value;
            if (this.URL.Contains("metrotwitauth"))
                this.SignIn();
            else
    ...
    private void FollowUs()
    {
      ...
      Friendship.CreateAsync(activeAccount.Tokens, "metrotwitapp", MetroTwitTwitterizer.CreateFriendshipOptions).ContinueWith((Action<Task<TwitterResponse<User>>>) (r =>
      {
    ...
    {
      if (SettingsData.Instance.Boon || App.AppState.Accounts.Count == 0)
      {
        Messenger.Default.Send<GenericMessage<object>>(new GenericMessage<object>((object) null), (object) ViewModelMessages.CloseCenterModalWindowHost);
        ...
        Messenger.Default.Send<DialogMessage>(new DialogMessage(string.Empty, (Action<MessageBoxResult>) (result => Messenger.Default.Send<GenericMessage<object>>(new GenericMessage<object>((object) Visibility.Visible), (object) ViewModelMessages.OverlayVisible))), (object) DialogType.NotPlusMultiAccount);
    }
    ...
    public void Authorise()
    {
      try
      {
        OAuth.RequestTokenAsync(SettingsData.Instance.TwitterConsumerKey, SettingsData.Instance.TwitterConsumerSecret, "http://metrotwitauth/").ContinueWith((Action<Task<OAuthTokenResponse>>) (r =>
        {
          if (r.Result != null)
          {
