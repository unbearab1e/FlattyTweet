// Type: MetroTwit.Extensions.CommonCommands
    ...
    public class CommonCommands
    {
        ...
        public static void Reply(MetroTwitStatusBase orig)
        {
            System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(() =>
            {
                ...
                if (orig.AdUrls == null || string.IsNullOrEmpty(orig.AdUrls.reply_url))
                    return;
                _140ProofService.HitAdUrl(orig.AdUrls.reply_url);
            }), DispatcherPriority.Background, new object[0]);
        }
        ...
        private static void internalChangeTheme(SharedResourceDictionary CustomDictionary)
        {
            ...
            else if (SettingsData.Instance.Accent == "Custom")
            {
                switch (SettingsData.Instance.AccentColour)
                {
                    case ColourAccent.DarkBlue:
                        ...
                        {
                            Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Accents/DarkBlue.xaml", UriKind.Absolute)
                        });
                        break;
                    case ColourAccent.Blue:
                        System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                        {
                            Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Accents/Blue.xaml", UriKind.Absolute)
                        });
                        break;
                    case ColourAccent.Purple:
                        System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                        {
                            Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Accents/Purple.xaml", UriKind.Absolute)
                        });
                        break;
                    case ColourAccent.Pink:
                        System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                        {
                            Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Accents/Pink.xaml", UriKind.Absolute)
                        });
                        break;
                    case ColourAccent.Red:
                        System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                        {
                            Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Accents/Red.xaml", UriKind.Absolute)
                        });
                        break;
                    case ColourAccent.Orange:
                        System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                        {
                            Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Accents/Orange.xaml", UriKind.Absolute)
                        });
                        break;
                    case ColourAccent.Green:
                        System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                        {
                            Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Accents/Green.xaml", UriKind.Absolute)
                        });
                        break;
                    case ColourAccent.Lime:
                        System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                        {
                            Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Accents/Lime.xaml", UriKind.Absolute)
                        });
                        break;
                    case ColourAccent.Silver:
                        System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                        {
                            Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Accents/Silver.xaml", UriKind.Absolute)
                        });
                        break;
                    default:
                        System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
            ...
            switch (string.IsNullOrEmpty(SettingsData.Instance.Theme) ? "Light" : SettingsData.Instance.Theme)
            {
                case "Light":
                    System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                    {
                        Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Light.xaml", UriKind.Absolute)
                    });
                    break;
                case "Dark":
                    System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                    {
                        Source = new Uri("pack://application:,,,/MetroTwit.Themes;component/Dark.xaml", UriKind.Absolute)
                    });
                    break;
            }
            StreamReader streamReader = new StreamReader(System.Windows.Application.GetResourceStream(new Uri("pack://application:,,,/MetroTwit.Themes;component/Shared/Theme.txt", UriKind.Absolute)).Stream);
            string str;
            while ((str = streamReader.ReadLine()) != null)
                System.Windows.Application.Current.Resources.MergedDictionaries.Add((ResourceDictionary)new SharedResourceDictionary()
                {
                    Source = new Uri(string.Format("pack://application:,,,/MetroTwit.Themes;component/Shared/{0}", (object)str), UriKind.Absolute)
                });
            streamReader.Close();
        ...
        internal static void CheckBoon(Action callback)
        {
            if (string.IsNullOrEmpty(SettingsData.Instance.PlusTransactionID) || string.IsNullOrEmpty(SettingsData.Instance.PlusEmail))
                return;
            Task task = new Task((Action)(() =>
            {
                string dateTime = DateTime.Now.Ticks.ToString();
                HttpWebRequest local_0 = (HttpWebRequest)WebRequest.Create(string.Format("https://appapi.metrotwit.com/verifyplus.php?id={0}&email={1}&datetime={2}", (object)SettingsData.Instance.PlusTransactionID, (object)HttpUtility.UrlEncode(SettingsData.Instance.PlusEmail), (object)dateTime));
                local_0.Proxy.Credentials = WebRequest.DefaultWebProxy.Credentials;
                local_0.UseDefaultCredentials = true;
                local_0.Timeout = 10000;
                HttpWebResponse response;
                try
                {
                    response = (HttpWebResponse)local_0.GetResponse();
                }
                catch (WebException exception_0)
                {
                    response = (HttpWebResponse)exception_0.Response;
                }
                if (response == null)
                    return;
                switch (response.StatusCode)
                {
                    case HttpStatusCode.OK:
                        System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(() =>
                        {
                            try
                            {
                                SettingsData.Instance.Boon = Extensions.Signature(string.Format("{0}{1}{2}", (object)SettingsData.Instance.PlusTransactionID, (object)SettingsData.Instance.PlusEmail, (object)dateTime)) == new StreamReader(response.GetResponseStream()).ReadToEnd().Replace("\n", "");
                            }
                            catch
                            {
                            }
                            if (!SettingsData.Instance.Boon)
                                Messenger.Default.Send<DialogMessage>(new DialogMessage(string.Empty, (Action<MessageBoxResult>)(o => { })), (object)DialogType.PlusVerifyError);
                            if (callback != null)
                                callback();
                    ...
                    case HttpStatusCode.Forbidden:
                        ...
                        {
                            Messenger.Default.Send<DialogMessage>(new DialogMessage(string.Empty, (Action<MessageBoxResult>)(o => { })), (object)DialogType.PlusVerifyError);
                            SettingsData.Instance.Boon = false;
                            if (callback != null)
                                callback();
                            App.AppState.AdColumnChanged();
                        }), DispatcherPriority.Background, new object[0]);
                    ...
                    default:
                        System.Windows.Application.Current.Dispatcher.BeginInvoke((Action)(() =>
                        {
                            Messenger.Default.Send<DialogMessage>(new DialogMessage(string.Empty, (Action<MessageBoxResult>)(o => { })), (object)DialogType.MetroTwitServerError);
                            SettingsData.Instance.Boon = false;
                            if (callback != null)
                                callback();
                            App.AppState.AdColumnChanged();
        ...
        internal async static Task<bool?> Follow(string AdUrl, MetroTwitUser User, decimal TwitterAccountID, string FollowText = "following...")
        {
            ...
            try
            {
                ...
                if (FollowText == "following...")
                {
                    ...
                    {
                        _140ProofService.HitAdUrl(AdUrl);
                    }
